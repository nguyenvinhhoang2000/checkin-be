...
router.get("/", verifyToken, async (req, res) => {
    try {
        const targetGroups = await CustomerGroup.find({ user: req.userId }).sort({ createdAt: -1 });
...
router.post("/", verifyToken, async (req, res) => {
    try {
        ...
        if (groupCode) {
            const newTargetGroup = new CustomerGroup({
                user: req.userId,
                name,
                groupCode,
                description,
...
router.put("/:id", verifyToken, async (req, res) => {
    try {
        ...
        let customerGroup = CustomerGroup.findOne({
            user: req.userId,
            _id: req.params.id,
        })
...
router.delete("/:id", verifyToken, async (req, res) => {
    try {
        const customerDelete = await CustomerGroup.findOneAndDelete({
            user: req.userId,
            _id: req.params.id
        })
